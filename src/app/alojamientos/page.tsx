...
// Get all unique amenities from alojamiento categories
const getAllAmenities = () => {
  const allAmenities = new Set<string>();
  ALOJAMIENTO_CATEGORIES.forEach(category => {
...
function AlojamientosContent() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        console.log('Filtered alojamiento posts:', alojamientoPosts.length);
  ...
  const filteredPosts = posts.filter(post => {
    ...
    // Filter by max people (from specificFields)
    ...
    // Filter by amenities (from specificFields)
    ...
    return matchesSearch && matchesLocation() && matchesCategory && matchesPrice && 
           matchesMaxPeople && matchesAmenities;
  });
          ...
          <div className="flex-1">
            ...
            >
              <div className="flex items-center justify-between">
                <p className="text-gray-600 dark:text-gray-300">
                  {filteredPosts.length} {filteredPosts.length === 1 ? 'alojamiento encontrado' : 'alojamientos encontrados'}
                </p>
